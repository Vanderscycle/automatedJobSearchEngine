...
class indeedSpider(scrapy.Spider):
    ...
    def start_requests(self): 
        ...
        start_url = re.sub(replacePosition , str(indeedSpider.titlesSearch[indeedSpider.jobtitleIndex]), indeedSpider.start_url)
    ...
    def pageParser(self,response):
        ...
        for title, url in zip(
            ...
            # simple nlp to check whether the title fits what we want
        ...
        if indeedSpider.page <= 20: #testing purpose
        ...
        elif indeedSpider.page <= 20 and indeedSpider.jobtitleIndex <len(indeedSpider.titlesSearch):
            indeedSpider.jobtitleIndex +=1
            indeedSpider.page =0
            replacePosition = re.findall("\{position\}", indeedSpider.start_url)[0]
            start_url = re.sub(replacePosition , str(indeedSpider.titlesSearch[indeedSpider.jobtitleIndex]), indeedSpider.start_url)
            logger.log(f'heading to url{start_url}')
    ...
    def subPageParser(self,response):
        ...
        if not response.css('.jobsearch-CompanyInfoWithoutHeaderImage a::text').get():
            job['company'] = response.css('.jobsearch-CompanyInfoWithoutHeaderImage div::text').get()
        else:
            job['company'] = response.css('.jobsearch-CompanyInfoWithoutHeaderImage a::text').get()
